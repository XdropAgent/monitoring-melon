...
<head>
  ...
  <title>Dashboard Monitoring Melon</title>
        ...
        <div class="main-image-wrap">
          ...
          <div class="img-overlay">
            <div class="img-meta">
              ...
              <span>TEMP: <span class="val" id="overlay-suhu">-</span></span>
              <span>HUM: <span class="val" id="overlay-kelembapan">-</span></span>
              <span>SOIL: <span class="val" id="overlay-soil">-</span></span>
              <span>DATE: <span class="val" id="overlay-date">n/a</span></span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="gauges-row">
          <div class="gauge-wrap">
            <svg class="gauge-svg" viewBox="0 0 80 80">
              <circle class="gauge-bg" cx="40" cy="40" r="32" stroke-dasharray="201" stroke-dashoffset="0"/>
              <circle id="gauge-suhu-circle" cx="40" cy="40" r="32" fill="none" stroke-width="8" stroke-linecap="round" stroke="#e53e3e" stroke-dasharray="201" stroke-dashoffset="201"/>
              <g transform="rotate(90, 40, 40)">
                <text id="gauge-suhu-text" class="gauge-val-text" x="40" y="37">-</text>
          ...
          <div class="gauge-wrap">
            <svg class="gauge-svg" viewBox="0 0 80 80">
              <circle class="gauge-bg" cx="40" cy="40" r="32" stroke-dasharray="201" stroke-dashoffset="0"/>
              <circle id="gauge-hum-circle" cx="40" cy="40" r="32" fill="none" stroke-width="8" stroke-linecap="round" stroke="#00b5a3" stroke-dasharray="201" stroke-dashoffset="201"/>
              <g transform="rotate(90, 40, 40)">
                <text id="gauge-hum-text" class="gauge-val-text" x="40" y="37">-</text>
          ...
          <div class="gauge-wrap">
            <svg class="gauge-svg" viewBox="0 0 80 80">
              <circle class="gauge-bg" cx="40" cy="40" r="32" stroke-dasharray="201" stroke-dashoffset="0"/>
              <circle id="gauge-soil-circle" cx="40" cy="40" r="32" fill="none" stroke-width="8" stroke-linecap="round" stroke="#2563eb" stroke-dasharray="201" stroke-dashoffset="201"/>
              <g transform="rotate(90, 40, 40)">
                <text id="gauge-soil-text" class="gauge-val-text" x="40" y="37">-</text>
...
<!-- ===== SCRIPT UTAMA ===== -->
<script>
  // ESP32 CAM IP — bisa di-override dari .env
  const ESP32_IP = '{{ config("app.esp32_cam_ip", "http://10.18.186.116") }}';
  ...
  function getWaktu(d) { return d.created_at || d.updated_at || null; }
  ...
  // Mini chart — empty init, Firebase will populate
  const miniChartCtx = document.getElementById('miniChart').getContext('2d');
  const miniChart = new Chart(miniChartCtx, {
    type: 'line',
    data: { labels: [], datasets: [
      { label:'Suhu', data:[], borderColor:'#e53e3e', backgroundColor:'rgba(229,62,62,.08)', tension:0.4, pointRadius:0, fill:true },
      { label:'Kelembapan', data:[], borderColor:'#00b5a3', backgroundColor:'rgba(0,181,163,.08)', tension:0.4, pointRadius:0, fill:true },
      { label:'Soil Moisture', data:[], borderColor:'#2563eb', backgroundColor:'rgba(37,99,235,.08)', tension:0.4, pointRadius:0, fill:true },
    ]},
...
<script type="module">
  ...
  import { getDatabase, ref, onValue, remove, query, orderByChild, limitToLast } from "https://www.gstatic.com/firebasejs/10.0.0/firebase-database.js";
  ...
  onValue(ref(db, '/sensor/history'), (snapshot) => {
    ...
    // Update mini chart with latest 20 points
    const ascending = [...data].reverse().slice(-20);
    const mLabels = ascending.map(d => { const w = d.created_at || d.updated_at || ''; return w ? w.slice(11, 16) : '-'; });
    miniChart.data.labels = mLabels;
    miniChart.data.datasets[0].data = ascending.map(d => d.suhu);
    miniChart.data.datasets[1].data = ascending.map(d => d.kelembapan);
    miniChart.data.datasets[2].data = ascending.map(d => d.soil);
    miniChart.update();

    // Riwayat chart: 50 data terlama
    const rAscending = [...data].reverse().slice(-50);
    const rLabels = rAscending.map(d => { const w = d.created_at || d.updated_at || ''; return w ? w.slice(11, 16) : '-'; });
    const rSuhu  = rAscending.map(d => d.suhu);
    const rHum   = rAscending.map(d => d.kelembapan);
    const rSoil  = rAscending.map(d => d.soil);
